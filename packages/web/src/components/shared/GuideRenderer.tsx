...
function ScreenshotWithHighlight({
  ...
  return (
    <div className="relative overflow-hidden rounded-lg border border-slate-200 bg-white">
      ...
      <div className="aspect-video w-full bg-gradient-to-br from-slate-50 via-slate-100/50 to-slate-50">
        {/* Mock UI elements */}
        <div className="relative h-full w-full">
          {/* Mock nav bar */}
          <div className="absolute inset-x-0 top-0 h-[8%] border-b border-slate-200/30 bg-white/80">
            <div className="flex h-full items-center px-4">
              <div className="h-2 w-16 rounded bg-slate-300/40" />
              <div className="ml-8 flex gap-4">
                {[1, 2, 3, 4].map((i) => (
                  <div key={i} className="h-1.5 w-12 rounded bg-slate-300/30" />
                ))}
              </div>
              <div className="ml-auto h-2.5 w-24 rounded bg-slate-300/30" />
            </div>
          </div>

          {/* Mock sidebar */}
          <div className="absolute bottom-0 left-0 top-[8%] w-[12%] border-r border-slate-200/20 bg-white/60">
            <div className="space-y-2 p-2 pt-4">
              {[1, 2, 3, 4, 5].map((i) => (
                <div key={i} className="h-2 w-full rounded bg-slate-300/20" />
              ))}
            </div>
          </div>
          ...
          <div className="absolute bottom-0 left-[12%] right-0 top-[8%] p-6">
            <div className="space-y-3">
              <div className="h-3 w-48 rounded bg-slate-300/25" />
              <div className="h-2 w-72 rounded bg-slate-300/15" />
              <div className="mt-6 grid grid-cols-3 gap-3">
                {[1, 2, 3].map((i) => (
                  <div
                    key={i}
                    className="h-20 rounded-lg border border-slate-200/20 bg-slate-100/30"
                  />
                ))}
              </div>
            </div>
          ...
          {step.highlightArea && isActive && (
            <>
              ...
              {/* Highlight cutout */}
              <div
                className="absolute z-10 rounded-lg ring-2 ring-green-600 shadow-lg shadow-indigo-500/20"
                style={{
              ...
              >
                <div className="absolute inset-0 rounded-lg bg-green-600/5" />
                {/* Label */}
                {step.elementLabel && (
                  <div className="absolute -bottom-8 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-md bg-green-600 px-2.5 py-1 text-[10px] font-medium text-white shadow-lg">
                    <MousePointer2 className="mr-1 inline h-2.5 w-2.5" />
                    {step.elementLabel}
                    <div className="absolute -top-1 left-1/2 h-2 w-2 -translate-x-1/2 rotate-45 bg-green-600" />
                  </div>
                )}
                {/* Pulse animation */}
                <div className="absolute -inset-1 animate-pulse rounded-lg ring-1 ring-green-500/30" />
              </div>
            </>
          )}
        </div>
      ...
      {/* Zoom button */}
      <button className="absolute right-2 top-2 rounded-lg bg-white/80 p-1.5 text-slate-500 opacity-0 transition-all hover:bg-slate-100 hover:text-slate-900 group-hover:opacity-100">
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  isActive
                    ? "border-indigo-500/30 bg-green-600/5"
